...
def main():
    ...
    idioma_destino = st.selectbox("Selecione o idioma de destino", [LANGUAGES[lang] for lang in LANGUAGES])

    if st.button("Traduzir"):
        if texto:
            try:
                resultado = translator.translate(texto, src=idioma_origem, dest=list(LANGUAGES.keys())[list(LANGUAGES.values()).index(idioma_destino)])
